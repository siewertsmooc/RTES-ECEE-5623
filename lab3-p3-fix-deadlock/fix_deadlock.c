/*
  This code has been adapted from the original version authored by Dr. Sam Siewart
  for ECEN5623 at the University of Colorado Boulder.

  See the original version here: https://github.com/siewertsmooc/RTES-ECEE-5623
*/
#include <pthread.h>
#include <stdio.h>
#include <sched.h>
...
struct timespec timeout_rsrcA = {0, 0};
struct timespec timeout_rsrcB = {0, 0};
...
void *grabRsrcs(void *threadp)
{
  ...
  if (threadIdx == THREAD_1)
  {
    ...
    // Create the timeout using a random value between 0 and 10
    clock_gettime(CLOCK_REALTIME, &timeout_rsrcB);
    timeout_rsrcB.tv_sec = timeout_rsrcB.tv_sec + (rand() % 10);
    rc = pthread_mutex_timedlock(&rsrcB, &timeout_rsrcB);
    if (rc != 0)
    {
      // Inidicates an error occured while trying to lock resource B
    ...
    {
      // rc = 0 means the call to pthread_mutex_timedlock was successful
      rsrcBCnt++;
  ...
  else
  {
    ...
    // Create the timeout using a random value between 0 and 10
    clock_gettime(CLOCK_REALTIME, &timeout_rsrcA);
    timeout_rsrcA.tv_sec = timeout_rsrcA.tv_sec + (rand() % 10);
    rc = pthread_mutex_timedlock(&rsrcA, &timeout_rsrcA);
    if (rc != 0)
    {
      // Inidicates an error occured while trying to lock resource B
      pthread_mutex_unlock(&rsrcB);
...
int main(int argc, char *argv[])
{
  ...
  // Initialize the seed for the random number generator which
  // will be used for a random backoff timer in blocking scenario.
  srand(time(NULL));
