...
void main(void)
{
    ...
    if(rc=sched_setscheduler(getpid(), SCHED_FIFO, &main_param) < 0) perror("******** WARNING: sched_setscheduler");
    ...
    do
    {
        ...
        current_ftime = (double)current_time.tv_sec + ((double)current_time.tv_nsec / 1000000000.0);
        //gettimeofday(&tv, (struct timezone *)0);
        syslog(LOG_CRIT, "SYSLOG_RT_TRC: My log message test @ tv.tv_sec %ld, tv.tv_nsec %ld, realtime %6.9lf\n", 
    	       current_time.tv_sec, current_time.tv_nsec, (current_ftime - start_ftime));
